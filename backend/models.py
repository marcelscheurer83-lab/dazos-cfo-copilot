...
class ForecastSnapshot(Base):
    ...
    nb_ai_forecast = Column(Float, nullable=True)          # actuals + AI-weighted pipeline (legacy; no IQ)
    exp_ai_forecast = Column(Float, nullable=True)
    # Full AI forecast: actuals + AI-weighted pipeline + in-quarter est (matches UI "Forecast (AI)")
    nb_ai_adjusted_forecast = Column(Float, nullable=True)
    exp_ai_adjusted_forecast = Column(Float, nullable=True)
    total_ai_adjusted_forecast = Column(Float, nullable=True)
    # Tier-weighted forecast: actuals + tier-weighted pipeline + in-quarter est (matches UI "Forecast (Tier)")
    nb_pipeline_tier_weighted = Column(Float, nullable=True)
    exp_pipeline_tier_weighted = Column(Float, nullable=True)
    nb_tier_forecast = Column(Float, nullable=True)
    exp_tier_forecast = Column(Float, nullable=True)
    total_tier_forecast = Column(Float, nullable=True)
    created_at = Column(DateTime, server_default=func.now())
